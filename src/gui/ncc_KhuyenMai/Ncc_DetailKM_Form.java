...
public class Ncc_DetailKM_Form extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Ncc_DetailKM_Form window = new Ncc_DetailKM_Form();
	 ...
	 */
	public Ncc_DetailKM_Form() {
		initialize();
	}
	
	public Ncc_DetailKM_Form(String tenLogin, String maKM) {
	...
	private void initialize() {
		...
		setFrame(new JFrame());
		getFrame().setBounds(100, 100, 636, 379);
...
//		btnThem.setIcon(new ImageIcon(img));
		...
		btnSua.setBounds(444, 258, 165, 43);
		...
		frm.getContentPane().add(lblTenlogin);
				...
				btnQuayLai.setBounds(19, 258, 165, 43);
				frm.getContentPane().add(btnQuayLai);

				btnXoa.setBackground(Color.CYAN);
				btnXoa.setBounds(230, 258, 165, 43);
				...
				lblGiaKM.setBounds(9, 197, 123, 14);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(o.equals(btnSua)){ //Sua thong tin khuyen mai
			frm.dispose();
			Ncc_UpdateKM_Form ncc_UpdateKM = new Ncc_UpdateKM_Form(get_Login_Text,get_maKM_Text);
			ncc_UpdateKM.frame.setVisible(true);
		}else if(o.equals(btnQuayLai)){ // Quay lai
			frm.dispose();
			Ncc_Form_KM ncc_KM = new Ncc_Form_KM(get_Login_Text);
			ncc_KM.frame.setVisible(true);
		}else if(o.equals(btnXoa)){ // xoa khuyen mai
