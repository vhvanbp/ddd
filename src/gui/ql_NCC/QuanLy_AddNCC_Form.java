...
public class QuanLy_AddNCC_Form extends JFrame implements ActionListener {
	...
	String loaiTV = "Nha cung cap"; // loai la mac dinh
	//EndRegion
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					QuanLy_AddNCC_Form window = new QuanLy_AddNCC_Form();
	 ...
	 */
	public QuanLy_AddNCC_Form() {
		initialize();
	}
	
	public QuanLy_AddNCC_Form(String tenLogin) {
...
//		btnThem.setIcon(new ImageIcon(img));
		...
		frm.getContentPane().add(lblTenlogin);
				...
				lblCongTy.setBounds(346, 130, 112, 14);
				frm.getContentPane().add(lblCongTy);
				
				txtCongTy = new JTextField();
				txtCongTy.setColumns(10);
				txtCongTy.setBounds(468, 127, 161, 20);
	...
	public void setFrame(JFrame frame) {
		this.frm = frame;
		frm.setTitle("Thêm nhà cung cấp");
...
//			int countTV = tvBAL.Count("ThanhVien"); // dem thanh vien 
...
//				JOptionPane.showMessageDialog(null, "Tài khoản " + taiKhoan + " đã tồn tại, hãy nhập tài khoản khác");
				...
				int dialogResult = JOptionPane.showConfirmDialog(this, "Bạn có muốn thêm nhà cung cấp này?"
																	, "Title on Box"
																	, dialogButton);
				if(dialogResult == 0) {
					NhaCungCap ncc = new NhaCungCap(mancc, tenTV, loaiTV, email, 
													sdt, taiKhoan, password, congTy);
					if(nccBAL.create(ncc)){
						JOptionPane.showMessageDialog(null, "Bạn vừa thêm nhà cung cấp!");
