...
public class Admin_AddQLV_Form extends JFrame implements ActionListener {
	...
	String loaiTV = "Quan li"; // loai la mac dinh
	//EndRegion
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Admin_AddQLV_Form window = new Admin_AddQLV_Form();
	 ...
	 */
	public Admin_AddQLV_Form() {
		initialize();
	}
	
	public Admin_AddQLV_Form(String tenLogin) {
...
//		btnThem.setIcon(new ImageIcon(img));
		...
		JLabel lblNhpTn = new JLabel("Nhập tên quản lí:");
		lblNhpTn.setHorizontalAlignment(SwingConstants.RIGHT);
		lblNhpTn.setBounds(20, 133, 99, 14);
		frm.getContentPane().add(lblNhpTn);
		
		txtTenTV = new JTextField();
		txtTenTV.setBounds(129, 130, 161, 20);
		...
		frm.getContentPane().add(lblTenlogin);
				...
				JLabel lblMoTaTV = new JLabel("Nhập mô tả:");
				lblMoTaTV.setHorizontalAlignment(SwingConstants.RIGHT);
				lblMoTaTV.setBounds(20, 195, 99, 14);
				frm.getContentPane().add(lblMoTaTV);
				
				txtMoTaTV = new JTextField();
				txtMoTaTV.setColumns(10);
				txtMoTaTV.setBounds(129, 192, 161, 20);
				frm.getContentPane().add(txtMoTaTV);
				...
				lblNhpEmail.setBounds(20, 164, 99, 14);
				frm.getContentPane().add(lblNhpEmail);
				
				txtEmail = new JTextField();
				txtEmail.setColumns(10);
				txtEmail.setBounds(129, 161, 161, 20);
				frm.getContentPane().add(txtEmail);
				
				txtTaiKhoan = new JTextField();
				txtTaiKhoan.setColumns(10);
				txtTaiKhoan.setBounds(468, 130, 161, 20);
				...
				lblNhpTK.setBounds(346, 133, 112, 14);
				...
				lblNhpPass.setBounds(346, 164, 112, 14);
				frm.getContentPane().add(lblNhpPass);
				
				txtPassword = new JTextField();
				txtPassword.setColumns(10);
				txtPassword.setBounds(468, 161, 161, 20);
	...
	public void setFrame(JFrame frame) {
		this.frm = frame;
		frm.setTitle("Thêm quản lí viên");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(o.equals(btnThem)){ //Them thong tin nha cung cap
			//Region - get text from ui
			int newID = UI_Helpers.addNewID("ThanhVien"); //tao index tiep theo cho list
			String mancc = "TV" + newID;
			String tenncc =	txtTenTV.getText();
			String email =	txtEmail.getText();
			String sdt =	txtSDT.getText();
			String taiKhoan =	txtTaiKhoan.getText();
			String password =	txtPassword.getText();
			String moTaTV = txtMoTaTV.getText();
			//EndRegion
			
			//Region - function create with condition
			if( tenncc.equals("") 	|| email.equals("") 	|| sdt.equals("")
			|| taiKhoan.equals("") 	|| password.equals("")	|| moTaTV.equals(""))
				...
				int dialogResult = JOptionPane.showConfirmDialog(this, "Bạn có muốn thêm quản lí viên này?"
																	, "Title on Box"
																	, dialogButton);
				if(dialogResult == 0) {
					QuanLiVien qlv = new QuanLiVien(mancc, tenncc, loaiTV, email, 
													sdt, taiKhoan, password, moTaTV);
					if(qlvBAL.create(qlv)){
						JOptionPane.showMessageDialog(null, "Bạn vừa thêm quản lí viên!");
