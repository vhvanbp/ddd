...
public class NhanVien_AddPhieuXuat_Form extends JFrame implements ActionListener {
	...
	NhanVien_Form_PhieuXuat qlLPT = new NhanVien_Form_PhieuXuat(get_Login_Text);
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NhanVien_AddPhieuXuat_Form window = new NhanVien_AddPhieuXuat_Form();
	 ...
	 */
	public NhanVien_AddPhieuXuat_Form() {
		initialize();
	}
	
	public NhanVien_AddPhieuXuat_Form(String tenLogin) {
	...
	private void initialize() {
		...
		setFrame(new JFrame());
		getFrame().setBounds(100, 100, 556, 331);
...
//		btnThem.setIcon(new ImageIcon(img));
		...
		frm.getContentPane().add(lblTenlogin);
				...
				btnQuayLai.setBounds(14, 223, 165, 43);
				...
				JLabel lblNhpChiPh = new JLabel("Nhập chi phí:");
				lblNhpChiPh.setHorizontalAlignment(SwingConstants.RIGHT);
				lblNhpChiPh.setBounds(14, 169, 112, 14);
				frm.getContentPane().add(lblNhpChiPh);
				
				txtChiPhi = new JTextField();
				txtChiPhi.setColumns(10);
				txtChiPhi.setBounds(132, 164, 161, 24);
				frm.getContentPane().add(txtChiPhi);
...
//				JOptionPane.showMessageDialog(null, "Phụ tùng này đã có phiếu nhập!" );
				...
				int dialogResult = JOptionPane.showConfirmDialog(this, "Bạn có muốn thêm phiếu xuất này?"
																	, "Title on Box"
																	, dialogButton);
					
				if(dialogResult == 0) {
					PhieuXuat lpt = new PhieuXuat(mapx, maPT, soLuong, ngayXuatPhieu, chiphi);
						
					if(pxBAL.create(lpt)){
						JOptionPane.showMessageDialog(null, "Bạn vừa thêm phiếu xuất!");

						frm.dispose();
						qlLPT = new NhanVien_Form_PhieuXuat(get_Login_Text);
